locals {
  ...
  data_ssm_prefix          = "${var.contracts_ssm_root_prefix}/${var.environment}/data/${local.service_name}"
...
data "aws_ssm_parameter" "mongodb_secret_arn" {
  name = "${local.data_ssm_prefix}/mongodb/secret-arn"
}

data "aws_ssm_parameter" "redis_secret_arn" {
...
module "service" {
  ...
  secrets = {
    MONGODB_URI    = data.aws_ssm_parameter.mongodb_secret_arn.value
    REDIS_PASSWORD = data.aws_ssm_parameter.redis_secret_arn.value
  }

  secret_arns = [
    data.aws_ssm_parameter.mongodb_secret_arn.value,
    data.aws_ssm_parameter.redis_secret_arn.value,
  ]
