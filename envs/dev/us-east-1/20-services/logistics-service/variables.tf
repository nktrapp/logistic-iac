...
variable "redis_host" {
  description = "External managed Redis host used when create_redis is false."
  type        = string
  default     = ""
}

variable "redis_port" {
  description = "External managed Redis port used when create_redis is false."
  type        = number
  default     = 6379
}

variable "redis_password" {
  description = "External managed Redis password used when create_redis is false. Stored in Secrets Manager and injected as REDIS_PASSWORD."
  type        = string
  default     = ""
  sensitive   = true
}
...
# --- Redis Cloud (data-redis-cloud module) ---

variable "create_redis" {
  description = "Manage this service's Redis Cloud Essentials (free) database via Terraform. When false, redis_host/redis_port/redis_password are used as-is."
  type        = bool
  default     = false
}

variable "rediscloud_api_key" {
  description = "Redis Cloud API key. Required when create_redis is true."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "rediscloud_secret_key" {
  description = "Redis Cloud secret key. Required when create_redis is true."
  type        = string
  default     = ""
  sensitive   = true
}
...
variable "redis_subscription_name" {
  description = "Redis Cloud Essentials subscription name when create_redis is true. Match your existing subscription to import it."
  type        = string
  default     = "furb-logistics-logistics-service"
}

variable "redis_database_name" {
  description = "Redis Cloud database name when create_redis is true."
  type        = string
  default     = "logistics-cache"
}

variable "redis_plan_name" {
  description = "Redis Cloud Essentials plan name. 30MB is the free tier."
  type        = string
  default     = "30MB"
}

variable "redis_cloud_provider" {
  description = "Cloud provider backing the Redis Cloud Essentials plan."
  type        = string
  default     = "AWS"
}

variable "redis_cloud_region" {
  description = "Region for the Redis Cloud Essentials plan."
  type        = string
  default     = "us-east-1"
}
